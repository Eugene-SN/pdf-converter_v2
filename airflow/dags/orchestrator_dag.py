...
- ✅ Исправлена передача данных между этапами через правильные пути файлов
- ✅ Добавлены проверки существования промежуточных файлов  
- ✅ Исправлено маппирование путей между контейнерами
- ✅ Улучшена обработка ошибок и восстановление
- ✅ ИСПРАВЛЕНО: render_template_as_native_obj=True и | tojson в conf
- ✅ ИСПРАВЛЕНО: Bridge-файлы для надежной передачи путей между этапами
"""
...
def validate_orchestrator_input(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # ✅ ИСПРАВЛЕНО: Определение режима работы
        ...
        master_config = {
            ...
            # ✅ НОВОЕ: Пути для правильного маппирования
            'host_input_dir': '/mnt/storage/apps/pdf-converter/input_pdf',
            'host_output_zh_dir': '/mnt/storage/apps/pdf-converter/output_md_zh',
            'host_output_target_dir': f'/mnt/storage/apps/pdf-converter/output_md_{target_language}' if target_language not in ['original', 'zh'] else '/mnt/storage/apps/pdf-converter/output_md_zh',
...
def prepare_stage1_config(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        stage1_config = {
            ...
            # ✅ НОВОЕ: Указываем где сохранять результаты
...
def check_stage1_completion(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # ✅ ИСПРАВЛЕНО: Чтение bridge-файла от Stage 1
        ...
        # Фолбэк: поиск *_intermediate.json по маске если bridge не сработал
...
def prepare_stage2_config(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Конфигурация для Stage 2 с проверенным промежуточным файлом
        stage2_config = {
...
def check_stage2_completion(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not md_file_path:
            # Поиск любого недавнего MD файла
            import glob
            patterns = [
...
def prepare_stage3_config(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Stage 3 только если требуется перевод
...
def prepare_stage4_config(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Определяем финальный файл для валидации
        if master_config['translation_required']:
            # Если был перевод, валидируем переведенный файл
            translated_file = f"{master_config['host_output_target_dir']}/{master_config['timestamp']}_{master_config['filename'].replace('.pdf', '.md')}"
        else:
            # Если перевода не было, валидируем MD файл
            stage2_result = context['task_instance'].xcom_pull(task_ids='check_stage2_completion')
...
def finalize_orchestration(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # ✅ ИСПРАВЛЕНО: Определение финального результата
        final_output_path = None
        if master_config['translation_required']:
            final_output_path = f"{master_config['host_output_target_dir']}/{master_config['timestamp']}_{master_config['filename'].replace('.pdf', '.md')}"
        else:
            final_output_path = f"{master_config['host_output_zh_dir']}/{master_config['timestamp']}_{master_config['filename'].replace('.pdf', '.md')}"
...
# ✅ ИСПРАВЛЕНО: Задача 3: Запуск Stage 1 с | tojson
...
# ✅ НОВОЕ: Задача 4: Проверка завершения Stage 1
...
# Задача 5: Подготовка конфигурации Stage 2
...
# ✅ ИСПРАВЛЕНО: Задача 6: Запуск Stage 2 с | tojson
...
# ✅ НОВОЕ: Задача 7: Проверка завершения Stage 2
...
# Задача 8: Подготовка конфигурации Stage 3
...
# ✅ ИСПРАВЛЕНО: Задача 9: Запуск Stage 3 с | tojson
...
# Задача 10: Подготовка конфигурации Stage 4
...
# ✅ ИСПРАВЛЕНО: Задача 11: Запуск Stage 4 с | tojson
...
# Задача 12: Финализация оркестрации
...
# Задача 13: Финальное уведомление
...
# ✅ ИСПРАВЛЕНО: Правильная последовательность с проверками
